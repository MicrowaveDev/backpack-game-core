...
export function getReplayCombatantName(currentBattle, side, resolveName) {
  ...
  const snapshot = currentBattle.snapshots?.[side];
  const characterId = snapshot?.characterId || snapshot?.mushroomId;
...
export function formatReplayEvent(event, currentBattle, resolveName, resolveActionName, lang = 'ru') {
  ...
  switch (event.type) {
    case 'action': {
      const actorSnapshot = currentBattle?.snapshots?.[event.actorSide];
      const actorCharacterId = actorSnapshot?.characterId || actorSnapshot?.mushroomId;
      const actionName = (resolveActionName && actorCharacterId ? resolveActionName(actorCharacterId) : null) || event.actionName;
