...
export function createTelegramWebAppAdapter({ win = getDefaultWindow(), root = getDefaultRoot() } = {}) {
  ...
  function applyTelegramTheme(targetRoot = root) {
    ...
    if (theme.button_color) targetRoot.style.setProperty('--telegram-accent', theme.button_color);
    if (theme.secondary_bg_color) targetRoot.style.setProperty('--telegram-surface', theme.secondary_bg_color);
