...
export function useTouch(state, {
  ...
  document: documentRef = getDefaultDocument()
} = {}) {
  let dragEl = null;
  let ghostEl = null;
  let startX = 0;
  ...
  function findDropZone(x, y) {
    ...
    const el = documentRef?.elementFromPoint?.(x, y);
  ...
  function createGhost(el, x, y) {
    ...
    `;
    documentRef?.body?.appendChild(ghostEl);
    startX = x - rect.left;
    startY = y - rect.top;
  }
  ...
  function attachTouch(rootEl) {
    if (!rootEl) return;
    usingPointerEvents = !!win && 'PointerEvent' in win;
