...
//
// This file intentionally keeps current SQL/table behavior while product
// dependencies are injected. It should graduate to a stable module only after
// table names, catalog lookups, and validation policy become repository/config
// contracts shared by Mushroom and Meat.

function requiredDependency(name, value) {
  if (value == null) throw new Error(`Game run loadout port requires ${name}`);
  return value;
}

export function createGameRunLoadoutPort(options = {}) {
  const query = requiredDependency('query', options.query);
  const bagColumns = Number(options.bagColumns ?? 0);
  ...
  const getEffectiveShape = requiredDependency('getEffectiveShape', options.getEffectiveShape);
  const normalizeRotation = requiredDependency('normalizeRotation', options.normalizeRotation);
  const effectiveGridHeight = requiredDependency('effectiveGridHeight', options.effectiveGridHeight);
  const pieceCells = requiredDependency('pieceCells', options.pieceCells);
  const validateBagPlacement = requiredDependency('validateBagPlacement', options.validateBagPlacement);
  const validateGridItems = requiredDependency('validateGridItems', options.validateGridItems);
  const validateItemCoverage = requiredDependency('validateItemCoverage', options.validateItemCoverage);
  const isBag = requiredDependency('isBag', options.isBag);
...
function assignMissingBagAnchors(items) {
  ...
  for (const bag of bagRows) {
    ...
    outer: for (let y = 0; y <= maxY; y += 1) {
      for (let x = 0; x + cols <= bagColumns; x += 1) {
 ...
 */
async function insertLoadoutItem(client, params) {
 ...
 */
async function readCurrentRoundItems(client, gameRunId, playerId, roundNumber) {
 ...
 *
 */
async function copyRoundForward(client, gameRunId, playerId, fromRound, toRound) {
 ...
 */
async function deleteLoadoutItem(client, itemId) {
 ...
 */
async function deleteLoadoutItemByIdScoped(client, { rowId, gameRunId, playerId, roundNumber }) {
 ...
 */
async function deleteOneByArtifactId(client, gameRunId, playerId, roundNumber, artifactId) {
  const rows = await q(client,
 ...
 */
async function nextSortOrder(client, gameRunId, playerId, roundNumber) {
  const res = await q(client,
 ...
 */
async function insertRefund(client, { gameRunId, playerId, roundNumber, artifactId, refundAmount }) {
  await q(client,
 ...
 */
async function applyRunPlacements(client, gameRunId, playerId, roundNumber, items) {
