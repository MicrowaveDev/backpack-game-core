function requiredDependency(name, value) {
  if (value == null) throw new Error(`Battle service port requires ${name}`);
  return value;
}

async function getActiveSnapshotWithDeps(ctx, client, playerId) {
  const {
    bagColumns,
    effectiveGridHeight,
  ...
  const cumulativeIncome = roundIncome.slice(0, currentRound).reduce((sum, c) => sum + c, 0);
...
async function getDailyUsageWithDeps(ctx, client, playerId) {
  const currentDay = ctx.dayKey(new Date());
...
async function recordBattleWithDeps(ctx, client, { leftSnapshot, rightSnapshot, simulation, battleSeed, mode, opponentKind, ratedScope, challengeId, initiatorPlayerId }) {
  const { createId, getMushroomById, nowIso } = ctx;
  const battle = {
    id: createId('battle'),
    mode,
    initiatorPlayerId,
...
async function getBattleWithDeps(ctx, battleId, viewerPlayerId, existingClient = null) {
  const { parseJson, query } = ctx;
...
async function getBattleHistoryWithDeps(ctx, playerId, limit = 20) {
  const { query } = ctx;
  ...
  return Promise.all(result.rows.map((row) => getBattleWithDeps(ctx, row.id, playerId)));
}

export function createMushroomBattleServicePort(options = {}) {
  const ctx = {
    query: requiredDependency('query', options.query),
    getMushroomById: requiredDependency('getMushroomById', options.getMushroomById),
    getStarterPresetCost: requiredDependency('getStarterPresetCost', options.getStarterPresetCost),
    bagColumns: requiredDependency('bagColumns', options.bagColumns),
    roundIncome: requiredDependency('roundIncome', options.roundIncome),
    portraitUrl: requiredDependency('portraitUrl', options.portraitUrl),
    ...
    effectiveGridHeight: requiredDependency('effectiveGridHeight', options.effectiveGridHeight),
    validateLoadoutItems: requiredDependency('validateLoadoutItems', options.validateLoadoutItems),
    normalizeRotation: requiredDependency('normalizeRotation', options.normalizeRotation),
    resolveEquippedPortraitId: requiredDependency('resolveEquippedPortraitId', options.resolveEquippedPortraitId)
  };

  return {
    getActiveSnapshot: (client, playerId) => getActiveSnapshotWithDeps(ctx, client, playerId),
    getDailyUsage: (client, playerId) => getDailyUsageWithDeps(ctx, client, playerId),
    recordBattle: (client, params) => recordBattleWithDeps(ctx, client, params),
    getBattle: (battleId, viewerPlayerId, existingClient = null) =>
      getBattleWithDeps(ctx, battleId, viewerPlayerId, existingClient),
    getBattleHistory: (playerId, limit = 20) => getBattleHistoryWithDeps(ctx, playerId, limit)
  };
}
