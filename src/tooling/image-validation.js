...
export function validateImagePolicy(image, policy = {}) {
  ...
  if (expectedWidth != null && expectedHeight != null
    ...
    issues.push({ code: 'dimensions', message: `expected ${expectedWidth}x${expectedHeight}, got ${image.width}x${image.height}` });
  }
  ...
  for (const [key, actual, code] of checks) {
    ...
    if ((isMinimum && actual < limit) || (!isMinimum && actual > limit)) {
      issues.push({ code, message: `${code} ${percent(actual, 1)} is ${isMinimum ? 'below' : 'above'} ${percent(limit)}` });
    }
  }
  if (policy.minMargin != null) {
    ...
    if (Math.min(...margins) < policy.minMargin) {
      issues.push({ code: 'margin', message: `minimum transparent margin ${Math.min(...margins)}px is below ${policy.minMargin}px` });
    }
  }
  return { issues, stats };
}
